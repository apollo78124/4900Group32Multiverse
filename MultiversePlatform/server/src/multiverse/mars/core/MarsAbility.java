 ...
 * <p>
 * When an ability is triggered, a MarsAbility.State object is generated that
 * represents the current state of that instance of the ability. It progresses
 * through a sequence of states, dependent on the configuration of the
 * MarsAbility object.
 * <p>
 * As each state is entered or exitted, a method is called that can be overriden
 * to create different types of abilities.
 */
public class MarsAbility {
	 ...
	 * null objects are never equal to any other object including other null
	 * objects.
	 *
	...
	/**
	 * Sets the name of the ability. This is used to identify the ability, so it
	 * should be unique.
	 *
	...
	public enum TargetType {
		UNINIT, NONE, ENEMY, FRIEND, GROUP, SELF, AREA
	}
	 ...
	 */
	public void setCostProperty(String name) {
		costProp = name;
	}
	...
	/**
	 * Adds a cooldown to this ability. If any of the ability's cooldowns are
	 * activate on the mob attempting to activate the ability, it will not be able
	 * to activate.
	 *
	...
	/**
	 * Adds a reagent requirement to this ability. Reagents are items that are
	 * required to be present in inventory, and are consumed when the ability
	 * completes the ACTIVATING phase.
	 *
	...
	/**
	 * Adds a tool requirement to this ability. Tools are items that are required to
	 * be present in inventory. They are not consumed.
	 *
	...
	protected Map<ActivationState, Set<CoordinatedEffect>> coordEffectMap = new HashMap<ActivationState, Set<CoordinatedEffect>>();

	public String getCompleteAnimation() {
		return completeAnimation;
	}

	public void setCompleteAnimation(String anim) {
	...
	public void setCompleteSound(String sound) {
		completeSound = sound;
	}
	...
	public void completeActivation(State state) {
		...
		if (!reagentList.isEmpty()) {
			List<Long> items = InventoryClient.removeItems(combatInfo.getOwnerOid(), reagentList);
			if (items != null) {
...
//      "Ability " + state.getAbility() + " complete");
		...
		if (completeSound != null) {
			WorldManagerClient.SoundMessage soundMsg = new WorldManagerClient.SoundMessage(combatInfo.getOwnerOid());
	...
	public enum AbilityResult {
		SUCCESS, OUT_OF_RANGE, INVALID_TARGET, NOT_READY, TOO_CLOSE, OUT_OF_LOS, INSUFFICIENT_ENERGY, BAD_ASPECT,
		MISSING_REAGENT, MISSING_TOOL, BUSY
	}

	protected AbilityResult checkTarget(CombatInfo obj, CombatInfo target) {
		if (Log.loggingDebug)
			Log.debug("MarsAbility.checkTarget: obj=" + obj + " isUser=" + obj.isUser() + "target=" + target
					+ " attackable=" + ((target == null) ? "N/A" : target.attackable()));
		switch (targetType) {
		case NONE:
		case SELF:
	...
	public static void startAbility(MarsAbility ability, CombatInfo obj, CombatInfo target, MarsItem item) {
		if (Log.loggingDebug)
			Log.debug("MarsAbility.startAbility ability=" + ability.getName() + " obj=" + obj + " target=" + target
					+ " item=" + item);
	...
	public enum ActivationState {
		INIT, ACTIVATING, CHANNELLING, ACTIVATED, COMPLETED, CANCELLED, INTERRUPTED, FAILED
	}
	 ...
	 * 
	 * This variable is used for setting up how much experience each successful use
	 * of an ability is.
	 */
	int exp_per_use = 0;
	 ...
	 * -Experience system component-
	 * 
	 * Returns the amount of experience should be gained by successful use of this
	 * ability.
	 */
	 ...
	 * -Experience system component-
	 * 
	 * Sets the default max experience that will be needed before the ability gains
	 * a level.
	 */
