...
configMap = {
    "voice_server_host" : None,
    "voice_server_port" : None,
...
def _UpdateVoiceConfig():
    ...
    for param in configMap.keys():
        configStr = configStr + " " + param + " " + str(configMap[param]).lower()
    configStr = configStr + " player_oid " + str(ClientAPI.GetPlayerObject().OID)
    if voiceGroupOid > 0:
        configStr = configStr + " group_oid " + str(voiceGroupOid)
    ...
    if enableVoice:
        if not Voice.VoiceManagerRunning():
            ...
            RemoveEventHandler("VoiceAllocation",HandleVoiceAllocation)
            RemoveEventHandler("VoiceDeallocation",HandleVoiceDeallocation)              
            Voice.ConfigureVoiceManager(configStr, _ConnectCallback)
            _UpdatePushToTalk()
...
def _HandleVoiceServerResponse(properties):
    ...
    configMap["voice_server_host"] = ClientAPI.Voice.TranslateHostname(properties["host"])
    configMap["voice_server_port"] = properties["port"]
    _UpdateVoiceConfig()
    _UpdatePushToTalk()
